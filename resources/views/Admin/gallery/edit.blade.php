...
@section('content')
    ...
    <ol class="breadcrumb mb-4">
        ...
        <li class="breadcrumb-item"><a href="{{ route('admin.galleries') }}">Galleries</a></li>
        <li class="breadcrumb-item active">Edit</li>
    </ol>
    <form method="POST" enctype="multipart/form-data" action="{{ route('admin.galleries.update',$gallery->id) }}">
        @csrf
                ...
                <table style="width: 100%;">
                    <tr>
                        ...
                        <td><input id="description" type="text" name="description[{{ $item->id }}]"
                                class="form-control" value="{{ $gallery[$item->id]->description }}"></td>
                    </tr>
                </table>
            @endforeach
        </div>
        ...
        @foreach (App\Models\Admin\Language::all() as $item)
            <div class="form-group" id="imagelist{{ $item->id }}">
                <a href="javascript:open_popup('@filemanager_get_resource(dialog.php)?type=1&popup=1&field_id=images{{ $item->id }}&akey=@filemanager_get_key() ')"
                    ...
                    Images</a>
                <table style="width:100%;">
                    @if ($gallery[$item->id]->images)
                        @foreach ($gallery[$item->id]->images as $image)
                            <tr>
                                <td style="width:100px;">
